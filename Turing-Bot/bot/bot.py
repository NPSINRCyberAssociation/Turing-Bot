...
class Bot(commands.Bot):
    ...
    def load_cogs(self):
        ...
        cog_list = [os.path.splitext('.'.join(os.path.join(os.path.relpath(path), name).split('\\')))[0] for
                    path, sub_dirs, files in
                    os.walk('bot\\cogs') for
                    name in files if fnmatch(name, "*.py")]
    ...
    async def change_status(self):
        activities = [
            discord.Activity(name=f"{len(self.guilds)} servers and {sum([guild.member_count for guild in self.guilds])} users!", type=discord.ActivityType.watching),
